...
class CrunchCounterApp:
    ...
    def create_widgets(self):
        ...
        #crunch_frame = Frame(self.root, bg=BG_COLOR, relief="groove", highlightbackground=FG_COLOR, highlightthickness=10)
        #crunch_frame.place(x=0, y=0, width=1400, height=80)
        ...
        disclaimer_label1 = Label(disclaimer_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="Our calorie counting app is designed\nfor informational purposes only and\nshould not be considered medical advice.\n\nFor personalised guidance, please consult a\nqualified healthcare professional or dietician.")
        ...
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=3, column=0, sticky="w")
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=4, column=0, sticky="w")
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=5, column=0, sticky="w")
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=6, column=0, sticky="w")

        labels = ["Name:", "Age:", "Gender:", "Height:", "Weight:", "Activity:", "Email:"]
        for i, label_text in enumerate(labels):
            Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text=label_text).grid(row=i+7, column=0, sticky="w")

        self.user_name_entry = EntryWithPlaceholder(frame, "Full Name")
        self.user_name_entry.grid(row=7, column=1, sticky="w")
        ...
        age_chosen.grid(row=8, column=1, sticky="w")

        gender_names = [("Female"), ("Male")]
        self.gender = StringVar()
        style = ttk.Style()
        style.configure("TRadiobutton", background=BG_COLOR)
        ttk.Radiobutton(frame, text="Female", value="Female", var=self.gender, state="readonly", width=17, style="TRadiobutton").grid(row=9, column=1, sticky="w")
        ttk.Radiobutton(frame, text="Male", value="Male", var=self.gender, state="readonly", width=17, style="TRadiobutton").grid(row=9, column=2, sticky="w")

        self.height_entry = EntryWithPlaceholder(frame, "cm")
        self.height_entry.grid(row=10, column=1, sticky="w")

        self.weight_entry = EntryWithPlaceholder(frame, "kg")
        self.weight_entry.grid(row=11, column=1, sticky="w")
        ...
        activity_chosen.grid(row=12, column=1, columnspan=2, sticky="w")

        self.email_entry = EntryWithPlaceholder(frame, "email address")
        self.email_entry.grid(row=13, column=1, columnspan=2, sticky="w")
