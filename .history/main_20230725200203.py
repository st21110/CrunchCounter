...
class CrunchCounterApp:
    ...
    def create_widgets(self):
        ...
        def create_hollow_box(root, width, height, outer_color, inner_color):
            # Create the outer frame (larger)
            outer_frame = tk.Frame(root, width=width, height=height, bg=outer_color)
            outer_frame.pack(pady=10)

            # Create the inner frame (slightly smaller)
            inner_frame = tk.Frame(outer_frame, width=width-20, height=height-20, bg=inner_color)
