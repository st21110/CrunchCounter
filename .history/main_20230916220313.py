...
class CrunchCounterApp: #create class for app
    ...
    def create_get_started_frame(self, calorie_intake, user_data=None):
        ...
        a = f.add_subplot(111)
        self.bar = a.bar(["Calories Eaten", "Calories Left"], [0, 0], align="center")
    ...
    def create_bar_graph(self):
        fig, ax = plt.subplots()
        labels = ["CALS EATEN", "CALS LEFT"]
        heights = [0, self.calorie_intake]
        self.bar = ax.bar(labels, heights, color=['blue', 'green'])
        ax.set_ylabel('Calories')
        self.bar_chart_canvas = FigureCanvasTkAgg(fig, master=self.get_started_frame)
        self.bar_chart_canvas.get_tk_widget().pack(side=tk.TOP, fill=tk.BOTH, expand=True)
