...
class CrunchCounterApp: #create class for app
    ...
    def fetch_date(self, date_label, tkc):
    ...
    def create_logging_frame(self): #logs meal
        ...
        date_label = Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="DATE")
        date_label.place(x=400, y=550)

        date_button = Button(self.logging_frame, text="Select Date", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR)
        date_button.place(x=400, y=500)
        date_button.config(command=lambda: self.fetch_date(date_label, tkc))

        # Use Calendar widget for date selection
        tkc = Calendar(self.logging_frame, selectmode="day", year=2022, month=1, day=1)
        tkc.place(x=900, y=800)  # Adjust the placement as needed
