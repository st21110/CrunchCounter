...
class CrunchCounterApp: #create class for app
    ...
    def create_logging_frame(self): #logs meal
        ...
        save_var = IntVar()
        save_checkbox = Checkbutton(self.logging_frame, fg=FG_COLOR, bg=BG_COLOR, variable=save_var)
        save_checkbox.place(x=240, y=330)

        save_log_button = Button(self.logging_frame, text="SAVE LOG", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=self.save_log)
        save_log_button.place(x=285, y=400)
    ...
    def save_log(self):
        food_name_log = food_entry.get()  # Get the text from the food_entry field
        calories = caloriesint_entry.get()  # Get the text from the caloriesint_entry field
        quantity = quantity_entry.get()  # Get the text from the quantity_entry field
        save_meal = save_var.get()  # Get the value of the save_var checkbox
