...
class CrunchCounterApp: 
    last_recorded_date_file = "last_recorded_date.txt"
    ...
    def check_and_reset_calories(self):
        #Load the last recorded date from a file or storage
        last_recorded_date = self.load_last_recorded_date()

        #Get the current date
        current_date = datetime.now().date()

        if current_date > last_recorded_date:
            #Reset calories eaten to zero
            self.calorie_intake = 0

            #Update the last recorded date to the current date
            self.save_last_recorded_date(current_date)

    def load_last_recorded_date(self):
        try:
            with open(self.last_recorded_date_file, "rb") as file:
                last_recorded_date = pickle.load(file)
            return last_recorded_date
        except FileNotFoundError:
            return None

    def save_last_recorded_date(self, date):
        with open(self.last_recorded_date_file, "wb") as file:
