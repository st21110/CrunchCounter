...
class CrunchCounterApp: 
    ...
    def switch_to_get_started(self, user_data=None):
        ...
        calories_eaten = user_data.get('calories_eaten', 0)
        calories_left = user_data.get('calories_left', self.calorie_intake) - calories_eaten
        
        # Update the calories left label with conditional check for negative value
        if calories_left < 0:
            self.calories_left_label.config(text=f"Calories Left: {calories_left}", fg="red")
        else:
            self.calories_left_label.config(text=f"Calories Left: {calories_left}", fg=FG_COLOR)
