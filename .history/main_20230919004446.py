...
class CrunchCounterApp: #create class for app
    ...
    def create_get_started_frame(self, calorie_intake, user_data):
        ...
        button_width = 370
        button_height = 100
        snack_image = breakfast_image.subsample(int(snack_image.width() / button_width), int(breakfast_image.height() / button_height))
        save_snack_button = Button(frame, image=breakfast_image, command=self.switch_to_logging, borderwidth=0)
        save_snack_button.image = breakfast_image 
