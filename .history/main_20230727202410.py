...
class CrunchCounterApp:
    ...
    def create_widgets(self):
        ...
        for i, label_text in enumerate(labels):
            Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text=label_text).grid(row=i+10, column=0, sticky="w")

        self.user_name_entry = EntryWithPlaceholder(frame, "Full Name")
        self.user_name_entry.grid(row=5, column=1, sticky="w")
        ...
        age_chosen.grid(row=6, column=1, sticky="w")

        gender_names = [("Female"), ("Male")]
        self.gender = StringVar()
        style = ttk.Style()
        style.configure("TRadiobutton", background=BG_COLOR)
        ttk.Radiobutton(frame, text="Female", value="Female", var=self.gender, state="readonly", width=17, style="TRadiobutton").grid(row=7, column=1, sticky="w")
        ttk.Radiobutton(frame, text="Male", value="Male", var=self.gender, state="readonly", width=17, style="TRadiobutton").grid(row=7, column=2, sticky="w")
