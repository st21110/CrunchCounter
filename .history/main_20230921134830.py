...
class CrunchCounterApp: 
    ...
    def create_get_started_frame(self, calorie_intake, user_data):
        ...
        today_date = Label(text=f"Today's date: {current_date}", font=HEADING_FONT, fg=FG_COLOR, bg=BG_COLOR)
        ...
        self.calorie_log_tree.place(x=20, y=350)
    ...
    def log_calories(self):
        ...
        # Get the calories eaten from the user's input (you can modify this part)
        calories_eaten = self.user_data[name]["calories_eaten"]  # Replace with the actual input

        # Check if the goal is achieved
        goal_achieved = calories_eaten <= self.calorie_intake

        # Add the log to the calorie logs list
        self.calorie_logs.append({"Date": current_date, "Calories Eaten": calories_eaten, "Goal Achieved": goal_achieved})
        ...
        # Update the user data (assuming you have the user's name)
        if name in self.user_data:
            self.user_data[name]["calorie_logs"] = self.calorie_logs
