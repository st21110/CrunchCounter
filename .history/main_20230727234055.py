...
BG_COLOR = "white"
HEADING_FONT = "Helvetica 25 bold"  # font size and type for headings
...
class CrunchCounterApp:
    ...
    def create_widgets(self):
        ...
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=1, column=0, sticky="w")
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=2, column=0, sticky="w")
        Label(frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=4, column=0, sticky="w")
        ...
        ttk.Radiobutton(frame, text="Female", value="Female", var=self.gender, state="readonly", width=6, style="TRadiobutton").grid(row=6, column=2, columnspan=9, sticky="w")
        ...
        activity_chosen = ttk.Combobox(frame, textvariable=self.activity, values=activity_amount,
                                       state="readonly", width=47, font=input_box_font)  # Adjust the width to fit the text
        ...
        calculate_button = Button(frame, text="Calculate !", font="Helvetica 20 bold", fg=FG_COLOR, bg=BG_COLOR, command=self.calculate)
        calculate_button.place(x=170, y=450)

        quit_button = Button(self.root, text="Quit", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=self.quit)
    ...
    def calculate(self):
        ...
        # Create a new window to display the result in fullscreen
        result_window = Tk()
        result_window.title("Crunch Counter")
        ...
        quit_button = Button(result_window, text="Quit", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=result_window.destroy)
        quit_button.place(x=1200, y=15)

        crunch_label = Label(result_window, fg=FG_COLOR, font=MAIN_HEADING_FONT, bg=BG_COLOR, text="Crunch Counter")
        crunch_label.place(x=5, y=5)

        line_canvas = Canvas(result_window, bg=BG_COLOR, highlightthickness=0)
        line_canvas.place(x=0, y=70, width=1400, height=5)
        line_canvas.create_line(0, 0, 1400, 0, fill=FG_COLOR, width=5)

        # Create a frame to display calorie intake number
        calorie_frame = Frame(result_window, bg=BG_COLOR, relief="groove", highlightbackground=FG_COLOR, highlightthickness=10)
        ...
        result_label = Label(result_window, text=f"{name}'s\nRecommended\nCalorie Intake\n(Per Day):", font=CAL_FONT, fg=FG_COLOR, bg=BG_COLOR)
        result_label.place(x=20, y=200)

        calorie_number_label = Label(calorie_frame, text=f"{calorie_intake:.2f}kcal", font=CAL_FONT, fg=FG_COLOR, bg=BG_COLOR)
        calorie_number_label.pack(padx=50, pady=100)

        get_started_button = Button(result_window, text="Get Started ➭", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=lambda: self.get_started(result_window))
        get_started_button.place(x=1100, y=650)

    def get_started(self, result_window):
        # Create a new window to display the result in fullscreen
        home_window = Tk()
        home_window.title("Crunch Counter")
        home_window.config(bg=BG_COLOR)
        home_window.attributes("-fullscreen", True)

        quit_button = Button(home_window, text="Quit", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=home_window.destroy)
        quit_button.place(x=1200, y=15)

        crunch_label = Label(home_window, fg=FG_COLOR, font=MAIN_HEADING_FONT, bg=BG_COLOR, text="Crunch Counter")
        crunch_label.place(x=5, y=5)

        line_canvas = Canvas(home_window, bg=BG_COLOR, highlightthickness=0)
        line_canvas.place(x=0, y=70, width=1400, height=5)
        line_canvas.create_line(0, 0, 1400, 0, fill=FG_COLOR, width=5)

        # Create a frame for the content with scrolling capability
        content_frame = Frame(home_window, bg=BG_COLOR)
        content_frame.pack(fill=BOTH, expand=True)

        # Create a canvas with a vertical scrollbar
        canvas = Canvas(content_frame, bg=BG_COLOR, highlightthickness=0)
        scrollbar = Scrollbar(content_frame, command=canvas.yview, bg=FG_COLOR, troughcolor=BG_COLOR)
        canvas.configure(yscrollcommand=scrollbar.set)

        # Place the canvas and scrollbar on the content frame
        canvas.pack(side="left", fill="both", expand=True)
        scrollbar.pack(side="right", fill="y")

        # Create another frame to hold the actual content (user interface elements)
        main_frame = Frame(canvas, bg=BG_COLOR)
        canvas.create_window((0, 0), window=main_frame, anchor="nw")

        # Your user interface elements for the home window go here
        # For example:
        home_label = Label(main_frame, text="Welcome to the Home Window!", font=MAIN_HEADING_FONT, fg=FG_COLOR, bg=BG_COLOR)
        home_label.pack(pady=20)

        # Add more UI elements as needed

        # Update the scroll region to allow scrolling within the canvas
