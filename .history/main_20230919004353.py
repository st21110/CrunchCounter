...
class CrunchCounterApp: #create class for app
    ...
    def create_get_started_frame(self, calorie_intake, user_data):
        ...
        button_width = 370
        button_height = 100
        dinner_image = dinner_image.subsample(int(breakfast_image.width() / button_width), int(dinner_image.height() / button_height))
        save_dinner_button = Button(frame, image=breakfast_image, command=self.switch_to_logging, borderwidth=0)
        save_dinner_button.image = breakfast_image 
        save_dinner_button.place(x=5, y=320)
