...
class CrunchCounterApp: #create class for app
    ...
    def create_logging_frame(self): #logs meal
        ...
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=2, column=2, sticky="w")
        ...
        for i, label_text in enumerate(input_labels):
            Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text=label_text).grid(row=i+7, column=0, sticky="w")

        def open_calendar_popup():
            popup = Toplevel(self.root)  # Create a new popup window
            popup.title("Select Date")
            popup.geometry("300x300")  # Set the size of the popup window

            def confirm_date():
                selected_date = tkc.get_date()
                clear_label = Label()
                date_label.config(text=f"Selected Date: {selected_date}")
                popup.destroy()  # Close the popup window after selecting the date

            clear_label = Label(self.logging_frame, text="          ", font=HEADING_FONT, bg=BG_COLOR)
            clear_label.place(x=400, y=550)
            date_label = Label(self.logging_frame, text="Selected Date:", font=HEADING_FONT, fg=FG_COLOR, bg=BG_COLOR)
            date_label.place(x=400, y=550)

            tkc = Calendar(popup, selectmode="day", year=20233, month=1, day=1)
            tkc.pack(pady=10)

            confirm_button = Button(popup, text="Confirm", font=SMALL_FONT, fg="black", command=confirm_date)
            confirm_button.pack()

        open_calendar_button = Button(self.logging_frame, text="Open Calendar", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=open_calendar_popup)
        open_calendar_button.place(x=400, y=500)
