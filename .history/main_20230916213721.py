...
class CrunchCounterApp: #create class for app
    ...
    def save_log(self):
        ...
        if not food_name or not calories_log or not quantity:
        ...
        # Update the donut graph with the new values
        if self.donut_ax and self.donut_canvas:
            self.update_donut_graph(calories_eaten, calories_left)

        # Go back to the get started frame
