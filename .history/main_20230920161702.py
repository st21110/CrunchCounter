...
class CrunchCounterApp: 
    ...
    def create_get_started_frame(self, calorie_intake, user_data):
        ...
        def update_donut_graph(calories_eaten, calories_left):
            sizes = [calories_eaten, calories_left]
            labels = ['Calories Eaten', 'Calories Left']
            ...
            # Display the donut graph in a tkinter window
            plt.show()


        # Update the donut graph when the frame is created
        if user_data:
            self.update_donut_graph(user_data.get('calories_eaten', 0), user_data.get('calories_left', calorie_intake))
