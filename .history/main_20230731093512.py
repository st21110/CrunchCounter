...
class CrunchCounterApp:
    ...
    def calculate(self):
        ...
        calorie_number_label = Label(calorie_frame, text=f"{round(calorie_intake):.2f}kcal", font=CAL_FONT, fg=FG_COLOR, bg=BG_COLOR)
    ...
    def get_started(self, result_window):
        ...
        quit_button = Button(main_frame, text="Quit", font=SMALL_FONT, fg=FG_COLOR, bg=BG_COLOR, command=home_window.destroy)
        quit_button.pack(side="top", anchor="ne", padx=10, pady=10)
    
        crunch_label = Label(main_frame, fg=FG_COLOR, font=MAIN_HEADING_FONT, bg=BG_COLOR, text="Crunch Counter")
        crunch_label.pack(pady=20)
    
        line_canvas = Canvas(main_frame, bg=BG_COLOR, highlightthickness=0)
        line_canvas.pack(fill=X)
        line_canvas.create_line(0, 0, 1400, 0, fill=FG_COLOR, width=5)
