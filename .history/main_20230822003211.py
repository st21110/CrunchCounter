...
class CrunchCounterApp:
    ...
    def create_get_started_frame(self, calories_added):
        ...
        #number stuff
        calories_eaten = calories_eaten + calories_added
    ...
    def create_logging_frame(self):
        ...
        meal_entries = [] #list to hold the entries

        meal_label = Label(self.logging_frame, fg=FG_COLOR, font=MAIN_HEADING_FONT, bg=BG_COLOR, text="Lunch Log")
        meal_label.place(x=10 , y=100)

        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=1, column=3, sticky="w")
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=1, column=3, sticky="w")
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=3, column=3, sticky="w")
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=4, column=3, sticky="w")
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=5, column=3, sticky="w")
        Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text="").grid(row=6, column=3, sticky="w")

        input_labels = ["Date:", "Food Name:", "Calories:", "Quantity:", "Save this meal? "]
        for i, label_text in enumerate(input_labels):
            Label(self.logging_frame, fg=FG_COLOR, font=HEADING_FONT, bg=BG_COLOR, text=label_text).grid(row=i + 7, column=0, columnspan=3, sticky="w")

            # Create an entry widget for each label
            entry = Entry(self.logging_frame, font=input_box_font)
            entry.grid(row=i + 7, column=3, columnspan=2, sticky="w")
            meal_entries.append(entry)  # Add the entry widget to the list

        # You can access each entry widget by indexing the meal_entries list
        date_entry = meal_entries[0]
        food_name_entry = meal_entries[1]
        calories_entry = meal_entries[2]
        quantity_entry = meal_entries[3]
        save_meal_entry = meal_entries[4]

        date = date_entry.get()
        food_name = food_name_entry.get()
        calories_added = calories_entry.get()
        quantity_food = quantity_entry.get()
        save_meal = save_meal_entry.get()
