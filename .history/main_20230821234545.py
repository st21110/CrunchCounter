...
class CrunchCounterApp:
    ...
    def create_get_started_frame(self):
        ...
        # Create a FigureCanvasTkAgg widget and attach it to the "Get Started" frame
        canvas = FigureCanvasTkAgg(fig, master=self.get_started_frame)
        canvas.get_tk_widget().pack(fill='both', expand=True)

        # Initial data for the donut chart
        data = [30, 50, 20]
        ax.pie(data, labels=['Label 1', 'Label 2', 'Label 3'], autopct='%1.1f%%', startangle=90, wedgeprops={'edgecolor': 'gray'})
        ax.add_artist(plt.Circle((0, 0), 0.7, color='white'))  # Create the inner circle for the donut effect
        canvas.draw()  # Redraw the canvas
        
    def update_donut_graph(self, new_data):
        # Clear the existing graph
        self.donut_ax.clear()

        # Update the data in the graph
        self.donut_ax.pie(new_data, labels=['Label 1', 'Label 2', 'Label 3'], autopct='%1.1f%%', startangle=90, wedgeprops={'edgecolor': 'gray'})
        self.donut_ax.add_artist(plt.Circle((0, 0), 0.7, color='white'))  # Create the inner circle for the donut effect

        # Redraw the canvas to display the updated graph
        self.canvas.draw()

    def update_and_display_graph(self):
        # Update the data and call the update_donut_graph method
        new_data = [40, 30, 30]  # Update this with your new data
        self.update_donut_graph(new_data)
